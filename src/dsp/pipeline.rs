...
fn process_slab(
    ...
    let mut filtered = Array2::<f64>::zeros((valid_len, c));
    ...
    match whitener {
        Some(w) => w.apply(&filtered).mapv(|v| v as f32),
        None => filtered.mapv(|v| v as f32),
    }
}

fn read_calibration(src: &mut ChunkStream, calib_samples: usize) -> Array2<f64> {
    ...
    while total < calib_samples {
        match src.next() {
            Some(b) => {
                total += b.nrows();
                blocks.push(b.mapv(|v| v as f64));
            }
            None => break,
        }
    }
    let views: Vec<ArrayView2<f64>> = blocks.iter().map(|b| b.view()).collect();
...
pub struct Pipeline {
    ...
    params: ChainParams,
    whitener: Option<Whitener>,
    prev_tail: Option<Array2<f64>>,
    cur: Option<Array2<f64>>,
    primed: bool,
...
impl Pipeline {
    pub fn new(mut calib: ChunkStream, stream: ChunkStream, params: ChainParams) -> Self {
        let whitener = if params.whiten {
            ...
            let pad = params.padlen.min(calib_block.nrows().saturating_sub(1));
            let c = calib_block.ncols();

            let mut filtered = Array2::<f64>::zeros((calib_block.nrows(), c));
            let mut col = vec![0.0f64; calib_block.nrows()];
            for ch in 0..c {
                for (i, slot) in col.iter_mut().enumerate() {
                    *slot = calib_block[[i, ch]];
                }
                let y = filter::sosfiltfilt(&params.sos, &col, pad);
                for i in 0..y.len() {
                    filtered[[i, ch]] = y[i];
                }
            }
    ...
    fn read_f64(&mut self) -> Option<Array2<f64>> {
        self.stream.next().map(|b| b.mapv(|v| v as f64))
    }

    fn next_slab(&mut self) -> Option<(Array2<f64>, usize, usize)> {
        if !self.primed {
            self.cur = self.read_f64();
            self.primed = true;
        }
        let cur = self.cur.take()?;
        let next = self.read_f64();
        ...
        let left = self
            .prev_tail
            .take()
            .unwrap_or_else(|| Array2::<f64>::zeros((0, c)));
        let right = match &next {
            Some(n) => {
                let r = m.min(n.nrows());
                n.slice(s![..r, ..]).to_owned()
            }
            None => Array2::<f64>::zeros((0, c)),
        };
