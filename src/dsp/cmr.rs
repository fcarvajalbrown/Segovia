...
pub fn common_median_reference(chunk: &mut Array2<f64>, scratch: &mut Vec<f64>) {
...
mod tests {
    ...
    fn subtracts_per_sample_median_odd_channels() {
        let mut chunk = array![[1.0, 2.0, 3.0], [10.0, 40.0, 70.0]];
        let mut scratch = Vec::new();
        common_median_reference(&mut chunk, &mut scratch);
        assert_eq!(chunk, array![[-1.0, 0.0, 1.0], [-30.0, 0.0, 30.0]]);
    }

    #[test]
    fn even_channels_average_two_middles() {
        let mut chunk = array![[1.0, 2.0, 3.0, 4.0]];
        let mut scratch = Vec::new();
        common_median_reference(&mut chunk, &mut scratch);
        assert_eq!(chunk, array![[-1.5, -0.5, 0.5, 1.5]]);
    }
}
