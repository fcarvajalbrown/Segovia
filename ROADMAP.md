...
- **Version:** `0.4.1` — three chunked, memory-bounded readers (SpikeGLX + Zarr + mtscomp `.cbin`) behind a shared `ChunkSource` trait, plus the streaming **bandpass → CMR → whiten** preprocessing chain (`reader.preprocess(...)`), live on crates.io + PyPI.
